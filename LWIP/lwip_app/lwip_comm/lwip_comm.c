...
//	lwipx->remoteip[3]=106;
	...
	lwipx->ip[0]=g_Setip[1];	
	lwipx->ip[1]=g_Setip[2];
	lwipx->ip[2]=g_Setip[3];
	lwipx->ip[3]=g_Setip[4];
	//默认子网掩码:255.255.255.0
	lwipx->netmask[0]=g_Setnetmask[0];	
	lwipx->netmask[1]=g_Setnetmask[1];
	lwipx->netmask[2]=g_Setnetmask[2];
	lwipx->netmask[3]=g_Setnetmask[3];
	//默认网关:192.168.1.1
	lwipx->gateway[0]=g_Setgateway[0];	
	lwipx->gateway[1]=g_Setgateway[1];
	lwipx->gateway[2]=g_Setgateway[2];
	lwipx->gateway[3]=g_Setgateway[3];	
...
u8 lwip_comm_init(void)
{
	...
	printf("网卡MAC地址为:................%02X-%02X-%02X-%02X-%02X-%02X\r\n",lwipdev.mac[0],lwipdev.mac[1],lwipdev.mac[2],lwipdev.mac[3],lwipdev.mac[4],lwipdev.mac[5]);
	...
	while(1)
	{
		...
		if(g_Setip[0] != 0xAA)
		{
			...
			if(ip!=0)   					//当正确读取到IP地址的时候
			{
				...
				printf("网卡en的MAC地址为:................%02X-%02X-%02X-%02X-%02X-%02X\r\n",lwipdev.mac[0],lwipdev.mac[1],lwipdev.mac[2],lwipdev.mac[3],lwipdev.mac[4],lwipdev.mac[5]);
				//解析出通过DHCP获取到的IP地址
