...
/* 串行Flsh的片选GPIO端口  */
#define SF_RCC_CS 			RCC_APB2Periph_GPIOB
...
void bsp_InitSFlash(void)
{
    ...
    sf_ReadBuffer(buf, 10, 10); //读取从10号地址开始的十个数，判断是否有使用，没有使用的话，整个Flash擦除
    if((buf[0]!=0xAA)&&(buf[2]!=0xAA)&&(buf[4]!=0xAA)&&(buf[6]!=0xAA))
    {
        printf("检测出新Flash，准备擦除Flash\r\n");
        sf_EraseChip(); //擦除整个芯片
        printf("擦除整个Flash完成！\r\n");
